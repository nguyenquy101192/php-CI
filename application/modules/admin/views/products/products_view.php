...
<div class="info-button">
    ...
    <div class="button">
        <a href="<?php echo base_url(); ?>admin/products/insert">
            <button class="insert fa fa-plus">&nbsp;Add product</button>
        </a>
    </div>
    <div class="search-bar">
        <form accept-charset="utf-8" method="post" action="<?php echo base_url(); ?>admin/products/search">
            ...
            <select name="search_type" id="select-pro">
                <option value="1" <?php if (isset($search_type) && $search_type == 1) {
                    echo "selected";
                } ?>>Search by name
                </option>
                <option value="2" <?php if (isset($search_type) && $search_type == 2) {
                    echo "selected";
                } ?>>Search by id
                </option>
                <option value="3" <?php if (isset($search_type) && $search_type == 3) {
                    echo "selected";
                } ?>>Search by brand
                </option>
            </select>
...
<div class="table-center">
    <table border='1' id="tbl-product" class="result-table">
        <tr class="title-table">
            ...
            <td>Slider</td>
            <td>Product image</td>
            <td>Product name&nbsp;<a href="<?php echo $current_url . $page_number . '/pro_name/' . $next_order; ?>"
                                     class="fa fa-sort"></a></td>
            <td>List price&nbsp;<a href="<?php echo $current_url . $page_number . '/pro_list_price/' . $next_order; ?>"
                                   class="fa fa-sort"></a></td>
            <td>Sale price&nbsp;<a href="<?php echo $current_url . $page_number . '/pro_sale_price/' . $next_order; ?>"
                                   class="fa fa-sort"></a></td>
            <td>Description</td>
            <td>Brand&nbsp;<a href="<?php echo $current_url . $page_number . '/brand_id/' . $next_order; ?>"
                              class="fa fa-sort"></a></td>
            <td>Origin&nbsp;<a href="<?php echo $current_url . $page_number . '/pro_origin/' . $next_order; ?>"
                               class="fa fa-sort"></a></td>
        ...
        foreach ($listProduct as $key => $value) {
            ...
            <td class='align'><input type=checkbox <?php foreach ($listSlider as $key => $slider) {
                    if ($value['pro_id'] == $slider['pro_id']) {
                        echo "checked";
                                     ...
                                     onclick="window.location='<?php echo base_url() ?>admin/sliders/add_delete_slider?product_id=<?php echo $value['pro_id']; ?>'; return true;"/>
            </td>
            <td class="align">
                <img alt="IMAGE" src="<?php echo base_url() . "public/images/products/" . $value["pro_images"] ?>"
                     width='50' height='50'/>
            </td>
            ...
            <td class='align' style='width: 50px;'><a
                    href="<?php echo base_url(); ?>admin/products/update/<?php echo $value['pro_id']; ?>"
                    class="fa fa-pencil"></a></td>
            <td class='align' style='width: 50px;'><a
                    href="<?php echo base_url(); ?>admin/products/delete/<?php echo $value['pro_id']; ?>"
                    class="fa fa-trash-o" onclick="if(check_delete()==false) return false"></a></td>
            </tr><?php $stt++;
        } ?>
    </table>
